 ...
 */
function EDITOR_render_do(timestamp: number) {
    let renderKind;
    ...
    while (renderKind = EDITOR_renderKindArray.shift()) {
        switch (renderKind) {
            ...
            //case RenderKind.Cursor_flag_scrollIntoViewExplicit: // TODO: This is a silent error
            //    break;
            default:
                // the 'default case' is RenderKind.Cursor_n:
...
function EDITOR_render_do_cursor(timestamp: number, renderKind: RenderKind | number) {
    ...
    else if (entryZero === RenderKind.Cursor_flag_scrollIntoViewExplicit) {
        EDITOR_renderKindArray.shift();
...
function EDITOR_render_do_CreateViewport() {
    ...
    for (var i = 0; i < virtualCount; i++) {
        ...
        let indexLine = i + virtualIndexLine;
 ...
 * 
 * and I've moved all the 'virtualIndexLine' setting logic from here to the rAF
...
function EDITOR_render_do_Scroll(timestamp: number) {
    ...
    if (diff > 0 && diff < virtualCount) {
        ...
        lowerBound = virtualIndexLine;
        upperBound = lowerBound + virtualCount;

        EDITOR_sum_diffPositive += virtualCount;
...
function EDITOR_render_do_SyntaxHighlighting() {
    ...
    */
    ...
    let indexLine = virtualIndexLine;

    let i_bounded = 0;

    let bothButNotFull = false;

    if (total_diff >= virtualCount) {
        total_diff = virtualCount;
        ...
        if (bothButNotFull || local_sum_diffNegative > 0) {
            ...
            indexLine = indexLine + virtualCount - 1;
...
//
    ...
    //You can guess that is diff < virtualCount
...
function EDITOR_state_setText(text: string, localFileStartsWithBom: boolean, textSourceIdentifier: string, FORMATTED_textSourceIdentifier: string, extensionKind: ExtensionKind, lineEndString: string | null) {
    ...
    // ...in the virtual index line equal to the count of lines being displayed, i.e.: set virtual index line to 'virtualCount'...
...
function EDITOR_finalizeEdit(cursor: EDITOR_Cursor) {
    ...
    if (EDITOR_cursorList.length === 1) {
        if (indexLine_editOccurredOn >= 0 && indexLine_editOccurredOn < EDITOR_lineEndPositionList.count) {
            if (cached_EDITOR_gutter.children.length === virtualCount &&
                cached_EDITOR_textElement.children.length === virtualCount) {
                    ...
                    let beltIndexLine = (indexLine_editOccurredOn + offsetLine) - virtualIndexLine;
...
function EDITOR_finalizeEdit_InsertLtr(cursor: EDITOR_Cursor, indexLine_editOccurredOn: number) {
    ...
    for (var i = 0; i < EDITOR_trackedSyntaxList.count_abstract; i++) {
        ...
        if (cursor.editPosition <= EDITOR_pooledTrackedSyntax_start) {
            EDITOR_trackedSyntaxList.setStart(i, EDITOR_pooledTrackedSyntax_start + cursor.editLength);
        }
        else if (EDITOR_pooledTrackedSyntax_trackedSyntaxKind === TrackedSyntaxKind.Comment &&
                cursor.editPosition === EDITOR_pooledTrackedSyntax_start + 1) {

            // TODO: Insertion of '*' probably shouldn't remove.
            EDITOR_trackedSyntaxList.removeAt(i, 1);
        }
        else if (cursor.editPosition > EDITOR_pooledTrackedSyntax_start && cursor.editPosition < EDITOR_pooledTrackedSyntax_start + EDITOR_pooledTrackedSyntax_length) {
            EDITOR_trackedSyntaxList.setLength(i, EDITOR_pooledTrackedSyntax_length + cursor.editLength);
        }
    }
...
function EDITOR_finalizeEdit_IndentMore(cursor: EDITOR_Cursor, indexLine_editOccurredOn: number) {
    ...
    for (var lineI = startingIndex; lineI >= SMALL_lineAndColumnIndices_indexLine; lineI--) {
        ...
        if (linePos.start > EDITOR_pooledTrackedSyntax_start && linePos.start < EDITOR_pooledTrackedSyntax_start + EDITOR_pooledTrackedSyntax_length) {
            // # Then, you immediately know the trackedSyntax that encompasses the insertion (if it exists), so you increment its length by the text inserted on that respective line.
            EDITOR_trackedSyntaxList.setLength(trackedSyntaxReposition_i, EDITOR_pooledTrackedSyntax_length + 4);
        }
...
//
    ...
    for (var lineI = startingIndex; lineI >= SMALL_lineAndColumnIndices_indexLine; lineI--) {
        ...
        EDITOR_trackedSyntaxList.getElementAt(trackedSyntaxReposition_i);
        if (linePos.start > EDITOR_pooledTrackedSyntax_start && linePos.start < EDITOR_pooledTrackedSyntax_start + EDITOR_pooledTrackedSyntax_length) {
            EDITOR_trackedSyntaxList.setLength(trackedSyntaxReposition_i, EDITOR_pooledTrackedSyntax_length - innerRemoveCount);
        }
...
function EDITOR_finalizeEdit_DeleteLtr_BackspaceRtl_RemoveTextNoBatching(cursor: EDITOR_Cursor, indexLine_editOccurredOn: number) {
    ...
    for (var i = EDITOR_trackedSyntaxList.count_abstract - 1; i >= 0; i--) {
        ...
        else if (EDITOR_pooledTrackedSyntax_start >= cursor.editPosition && EDITOR_pooledTrackedSyntax_start < cursor.editPosition + cursor.editLength) {
            // TODO: This needs to remove more than 1 at a time
            EDITOR_trackedSyntaxList.removeAt(i, 1);
        }
        else if (EDITOR_pooledTrackedSyntax_trackedSyntaxKind === TrackedSyntaxKind.Comment &&
                (EDITOR_pooledTrackedSyntax_start + 1) >= cursor.editPosition && (EDITOR_pooledTrackedSyntax_start + 1) < cursor.editPosition + cursor.editLength) {
 ...
 */
function EDITOR_createSpansForLineOfText(div: HTMLElement, lineStart: number, lineEnd: number, trackedSyntax_I: number) {
	let childIndex = 0;
    ...
    if (lineStart === lineEnd) {
        ...
        for (; trackedSyntax_I < EDITOR_trackedSyntaxList.count_abstract;) {
            ...
            if (EDITOR_pooledTrackedSyntax_start >= lineEnd) {
                break;
            }
            ...
            if (EDITOR_pooledTrackedSyntax_start > substart) {
                let subend = EDITOR_pooledTrackedSyntax_start > lineEnd ? lineEnd : EDITOR_pooledTrackedSyntax_start; // probably a nonsense line of code given the previous if statements
            ...
            if (EDITOR_pooledTrackedSyntax_start + EDITOR_pooledTrackedSyntax_length <= lineEnd) {
                trackedSyntax_I++;
                continue;
            }
...
function walkLineUntilIndexColumn(cursor: EDITOR_Cursor) {
    ...
    // but 'beltIndexLine' is the output of the function, and a 'virtualIndex' variable is only needed temporarily
    // for the calculation. So by storing the 'virtualIndex' in 'beltIndexLine' at the start I skip a variable declaration.
    w_beltIndexLine = (cursor.indexLine + offsetLine) - virtualIndexLine;
    if (w_beltIndexLine >= ArrayFrom_textElement_children_length || w_beltIndexLine < 0) w_beltIndexLine = -1;
    else w_beltIndexLine = (w_beltIndexLine + EDITOR_beltIndexZero) % virtualCount;
...
function EDITOR_drawViewPort_FindTrackedSyntax_StartingIndex(indexLineAaa: number) {
    ...
    while (left <= right) {
        ...
        if (EDITOR_pooledTrackedSyntax_start + EDITOR_pooledTrackedSyntax_length > positionIndex) {
            indexLineBbb = mid;

            if (EDITOR_pooledTrackedSyntax_start === positionIndex) {
                break;
            }
            
            right = mid - 1;
        }
        else if (EDITOR_pooledTrackedSyntax_start + EDITOR_pooledTrackedSyntax_length <= positionIndex) {
            left = mid + 1;
        }
        else {
            return; // NaN
...
function EDITOR_createStyleForSelection(cursor: EDITOR_Cursor) {
    if (cursor.DRAWN_selectionAnchor !== cursor.selectionAnchor ||
        ...
        cursor.DRAWN_selection_virtualCount !== virtualCount ||
        cursor.DRAWN_selection_virtualIndexLine !== virtualIndexLine) {
        ...
        // # Virtualization
        if (startLine < virtualIndexLine) {
            startLine = virtualIndexLine;
            startColumn = 0;
        }
        let lastIndexLineBeingShown = virtualIndexLine + virtualCount - 1;
...
function EDITOR_createCursorLineBelow(event: KeyboardEvent) {
    ...
    EDITOR_render_request(RenderKind.Cursor_n + indexLastCursor);
    EDITOR_render_request(RenderKind.Cursor_n + indexLastCursor + 1);
...
function EDITOR_createCursorAtNextMatchSelection(event: KeyboardEvent) {
    ...
    if (prePosition != postPosition && postPosition != GLOBAL_findOverlay_isBeingShownDueToMultiCursorMatching_originMatchNumber) {
        ...
        cached_EDITOR_cursorListElement.appendChild(clone.caretRow);
        EDITOR_render_request(RenderKind.Cursor_n + indexOfPrimaryCursor); // no longer is indexOfPrimaryCursor because of the splice.
        ...
        EDITOR_render_request(RenderKind.Cursor_n + indexOfPrimaryCursor + 1); // no longer is indexOfPrimaryCursor because of the splice.
...
function EDITOR_onKeyDown_PageDown(event: KeyboardEvent) {
    ...
    if (event.ctrlKey) {
        EDITOR_primaryCursor.indexLine = virtualIndexLine + virtualCount;
        if (virtualCount > 1) {
...
function EDITOR_onKeyDown_PageUp(event: KeyboardEvent) {
    ...
    if (event.ctrlKey) {        
        EDITOR_primaryCursor.indexLine = virtualIndexLine;
        if (virtualCount > 1) {
            // this seems to more commonly have the cursor staying within the viewport rather than overlapping outside.
            EDITOR_primaryCursor.indexLine++;
        }
    ...
    for (let i = EDITOR_cursorList.length - 1; i >= 0; i--) {
        ...
        if (cursor.editRenderedDisplacement < cursor.editLength) {
            ...
            for (var lineI = startingIndex; lineI >= SMALL_lineAndColumnIndices_indexLine; lineI--) {
                ...
                // but 'beltIndexLine' is the output of the function, and a 'virtualIndex' variable is only needed temporarily
                // for the calculation. So by storing the 'virtualIndex' in 'beltIndexLine' at the start I skip a variable declaration.
                let beltIndexLine = (lineI + offsetLine) - virtualIndexLine;
                if (beltIndexLine >= ArrayFrom_textElement_children_length || beltIndexLine < 0) beltIndexLine = -1;
                else beltIndexLine = (beltIndexLine + EDITOR_beltIndexZero) % virtualCount;
...
function EDITOR_render_do_DuplicateOrPaste() {
    ...
    for (let i = EDITOR_cursorList.length - 1; i >= 0; i--) {
        ...
        if (cursor.editRenderedDisplacement < cursor.editLength || cursor.editKind === EditKind.Paste /* Paste has an editLength of 0 currently */) {
            ...
            // but 'beltIndexLine' is the output of the function, and a 'virtualIndex' variable is only needed temporarily
            // for the calculation. So by storing the 'virtualIndex' in 'beltIndexLine' at the start I skip a variable declaration.
            let beltIndexLine_current = ((cursor.indexLine) + offsetLine) - virtualIndexLine;
            if (beltIndexLine_current >= ArrayFrom_textElement_children_length || beltIndexLine_current < 0) beltIndexLine_current = -1;
            else beltIndexLine_current = (beltIndexLine_current + EDITOR_beltIndexZero) % virtualCount;

            // TODO: This is an awkward explicit inlining of 'EDITOR_indexLineTo_beltIndexLine'...
            // ...the initial declaration of 'let beltIndexLine' is assigned what I refer to as the "virtualIndex"
            // but 'beltIndexLine' is the output of the function, and a 'virtualIndex' variable is only needed temporarily
            // for the calculation. So by storing the 'virtualIndex' in 'beltIndexLine' at the start I skip a variable declaration.
            let beltIndexLine_first = ((virtualIndexLine) + offsetLine) - virtualIndexLine;
            if (beltIndexLine_first >= ArrayFrom_textElement_children_length || beltIndexLine_first < 0) beltIndexLine_first = -1;
            else beltIndexLine_first = (beltIndexLine_first + EDITOR_beltIndexZero) % virtualCount;
            ...
            // but 'beltIndexLine' is the output of the function, and a 'virtualIndex' variable is only needed temporarily
            // for the calculation. So by storing the 'virtualIndex' in 'beltIndexLine' at the start I skip a variable declaration.
            let beltIndexLine_last = ((virtualIndexLine + virtualCount - 1) + offsetLine) - virtualIndexLine;
            if (beltIndexLine_last >= ArrayFrom_textElement_children_length || beltIndexLine_last < 0) beltIndexLine_last = -1;
            else beltIndexLine_last = (beltIndexLine_last + EDITOR_beltIndexZero) % virtualCount;
...
function EDITOR_lineWasInsertedValidateGutter() {
    ...
    // 
    //if (cached_EDITOR_gutter.children.length > 0 && cached_EDITOR_gutter.children.length === virtualCount) {
...
function EDITOR_trackedSyntaxList_inefficientUpdateStartAndLength(indexPosition: number, insertionCount: number) {
    for (var i = 0; i < EDITOR_trackedSyntaxList.count_abstract; i++) {
        ...
        if (indexPosition <= EDITOR_pooledTrackedSyntax_start) {
            EDITOR_trackedSyntaxList.setStart(i, EDITOR_pooledTrackedSyntax_start + insertionCount);
        }
        else if (indexPosition > EDITOR_pooledTrackedSyntax_start && indexPosition < EDITOR_pooledTrackedSyntax_start + EDITOR_pooledTrackedSyntax_length) {
            EDITOR_trackedSyntaxList.setLength(i, EDITOR_pooledTrackedSyntax_length + insertionCount);
        }
    }
}
...
function EDITOR_render_do_EnterKey() {
    ...
    for (let i = EDITOR_cursorList.length - 1; i >= 0; i--) {
        ...
        if (cursor.editRenderedDisplacement < cursor.editLineFeedCount) {
            ...
            // but 'beltIndexLine' is the output of the function, and a 'virtualIndex' variable is only needed temporarily
            // for the calculation. So by storing the 'virtualIndex' in 'beltIndexLine' at the start I skip a variable declaration.
            let beltIndexLine_firstTilde = ((EDITOR_lineEndPositionList.count) + offsetLine) - virtualIndexLine;
            ...
            // but 'beltIndexLine' is the output of the function, and a 'virtualIndex' variable is only needed temporarily
            // for the calculation. So by storing the 'virtualIndex' in 'beltIndexLine' at the start I skip a variable declaration.
            let beltIndexLine_current = ((cursor.editIndexLine) + offsetLine) - virtualIndexLine;
            if (beltIndexLine_current >= ArrayFrom_textElement_children_length || beltIndexLine_current < 0) beltIndexLine_current = -1;
            else beltIndexLine_current = (beltIndexLine_current + EDITOR_beltIndexZero) % virtualCount;
            ...
            if (virtualCount <= 1 || cached_EDITOR_textElement.children.length !== virtualCount)
                shouldRenderEntireViewport = true;
...
function EDITOR_shiftLinesOfText_ToASmaller_IndexLine_byDistance(beltIndexLine_last: number, smallestBeltIndexLineToReceive: number, distance: number, local_virtualIndexLine?: number, local_virtualCount?: number) {
    ...
    if (!local_virtualCount) local_virtualCount = virtualCount;
...
function EDITOR_render_do_Resize(timestamp: number) {
    ...
    let remember_virtualCount = virtualCount;
    update_virtualCount();
    if (virtualCount !== remember_virtualCount) {
    ...
    for (let cursorI = EDITOR_cursorList.length - 1; cursorI >= 0; cursorI--) {
        ...
        if (cursor.editRenderedDisplacement < cursor.editLength) {
            ...
            if (indexTrackedSyntax < EDITOR_trackedSyntaxList.count_abstract) {
                ...
                if (EDITOR_pooledTrackedSyntax_start < EDITOR_lineEndPositionList.data[cursor.indexLine]) {
                ...
                for (let i = cursor.indexLine - 1; i >= 0; i--) {
                    ...
                    if (EDITOR_pooledTrackedSyntax_start < lineEndPosition &&
                        EDITOR_pooledTrackedSyntax_start + EDITOR_pooledTrackedSyntax_length > lineEndPosition) {
            ...
            for (var iVarDependent = cursor.indexLine; iVarDependent < EDITOR_lineEndPositionList.count - 1; iVarDependent++) {
                ...
                if (lineEnding >= cursor.editPosition && lineEnding < cursor.editPosition + editLength) {
                    ...
                    if (possibleTrackedSyntaxToSpanSingleLine) {
                        ...
                        else if (EDITOR_pooledTrackedSyntax_start + EDITOR_pooledTrackedSyntax_length <= EDITOR_lineEndPositionList.data[iVarDependent])
                            NOTlineEndBelongsToSyntax = true;
                        ...
                        if (NOTlineEndBelongsToSyntax) {
                            ...
                            if (indexTrackedSyntax < EDITOR_trackedSyntaxList.count_abstract) {
                                EDITOR_trackedSyntaxList.getElementAt(indexTrackedSyntax);
                                if (EDITOR_pooledTrackedSyntax_start < lineEnding &&
                                    EDITOR_pooledTrackedSyntax_start + EDITOR_pooledTrackedSyntax_length > lineEnding) {
                                        possibleTrackedSyntaxToSpanSingleLine = true;
                                }
                            }
                        }
            ...
            let largestDrawnIndexLine = virtualIndexLine + virtualCount - 1;
            ...
            if (linesRemovedCount > 0) {
                ...
                // but 'beltIndexLine' is the output of the function, and a 'virtualIndex' variable is only needed temporarily
                // for the calculation. So by storing the 'virtualIndex' in 'beltIndexLine' at the start I skip a variable declaration.
                let beltIndexLine_current = ((smallLineAndColumnIndices.indexLine + 1) + offsetLine) - virtualIndexLine;
                if (beltIndexLine_current >= ArrayFrom_textElement_children_length || beltIndexLine_current < 0) beltIndexLine_current = -1;
                else beltIndexLine_current = (beltIndexLine_current + EDITOR_beltIndexZero) % virtualCount;
...
comments from EDITOR_removeSelection(cursor) that may or may not be useful idk I just wanna get them out of the way.
    ...
    if (linesRemovedCount > 0) {
        ...
        let beltIndexLine_last = EDITOR_indexLineTo_beltIndexLine(virtualIndex + virtualCount - 1);
                    ...
                    if (remaining > 0) {
                        if (w_indexSpan >= w_div.children.length) {
                            ...
                            if (cursor.indexLine < EDITOR_lineEndPositionList.count - 1) {
                                ...
                                // but 'beltIndexLine' is the output of the function, and a 'virtualIndex' variable is only needed temporarily
                                // for the calculation. So by storing the 'virtualIndex' in 'beltIndexLine' at the start I skip a variable declaration.
                                let beltIndexLine_next = ((cursor.indexLine + 1) + offsetLine) - virtualIndexLine;
                                if (beltIndexLine_next >= ArrayFrom_textElement_children_length || beltIndexLine_next < 0) beltIndexLine_next = -1;
                                else beltIndexLine_next = (beltIndexLine_next + EDITOR_beltIndexZero) % virtualCount;
...
function EDITOR_stopTrackingIfTrackedSyntaxMadeToSpanSingleLine(cursor: EDITOR_Cursor) {
    ...
    if (indexTrackedSyntax < EDITOR_trackedSyntaxList.count_abstract) {
        EDITOR_trackedSyntaxList.getElementAt(indexTrackedSyntax);
        if (EDITOR_pooledTrackedSyntax_start < cursor.editPosition) {
