    ...
    while (pos < lineEnd) {
        switch (bytes[pos]) {
            ...
            case 47 /* FORWARDSLASH '/' */:
                if (bytes[pos + 1] === 47 /* FORWARDSLASH '/' */) {
                ...
                else if (bytes[pos + 1] === 42 /* ASTERISK '*' */) {
            ...
            case 61 /* EQUALS '=' */:
                ...
                if (pos > substart) {
                    if (bytes[pos - 1] === 61 /* EQUALS '=' */) {
                        shouldSkipContiguous = true;
                    }
                    else if (bytes[pos - 1] === 33 /* BANG '!' */) {
                        shouldSkipContiguous = true;
                    }
                    else if (bytes[pos - 1] === 60 /* OPENBRACKET '[' */) {
                        shouldSkipContiguous = true;
                    }
                    else if (bytes[pos - 1] === 62 /* CLOSEBRACKET ']' */) {
                        shouldSkipContiguous = true;
                    }
                }
                else {
                ...
                if (!shouldSkipContiguous) {
                    if (pos < lineEnd && bytes[pos + 1] === 61 /* EQUALS '=' */) {
                        shouldSkipContiguous = true;
                    }
                }
                ...
                if (shouldSkipContiguous) {
                    ...
                    pos++;
                    // skip contiguous
                    while (pos < lineEnd && bytes[pos] === 61 /* EQUALS '=' */) {
                        pos++;
                    }
                    continue;
                }
                    ...
                    pos++;
                    substart++;
                    if (pos < lineEnd && bytes[pos] === 62 /* CLOSEBRACKET ']' */) {
                        textContent = '=>';
                        pos++;
                        substart++;
                    }
            ...
            case 43 /* PLUS '+' */:
                ...
                // TODO: This contiguous skipping logic isn't working for every switch case?
                shouldSkipContiguous = pos > substart && bytes[pos - 1] === 43 /* PLUS '+' */;
                if (!shouldSkipContiguous) {
                    if (pos < lineEnd) {
                        if (bytes[pos + 1] === 43 /* PLUS '+' */) {
                            textContent = '++';
                        }
                        else if (bytes[pos + 1] === 61 /* EQUALS '=' */) {
                            textContent = '+=';
                        }
                        else {
                            shouldSkipContiguous = true;
                ...
                if (shouldSkipContiguous) {
                    ...
                    pos++;
                    // skip contiguous
                    while (pos < lineEnd && bytes[pos] === 43 /* PLUS '+' */) {
                        pos++;
                    }
                    continue;
                }
            ...
            case 45 /* MINUS '-' */:
                ...
                // TODO: This contiguous skipping logic isn't working for every switch case?
                shouldSkipContiguous = pos > substart && bytes[pos - 1] === 45 /* MINUS '-' */;
                if (!shouldSkipContiguous) {
                    if (pos < lineEnd) {
                        if (bytes[pos + 1] === 45 /* MINUS '-' */) {
                            textContent = '--';
                        }
                        else if (bytes[pos + 1] === 61 /* EQUALS '=' */) {
                            textContent = '-=';
                ...
                if (shouldSkipContiguous) {
                    ...
                    pos++;
                    // skip contiguous
                    while (pos < lineEnd && bytes[pos] === 45 /* MINUS '-' */) {
                        pos++;
                    }
                    continue;
                }
