...
const os = require('os');
import * as lspTypes from './lspTypes';
...
let workspaceDirectories: WorkspaceDirectory[] | null = null;

/** Relates to LSP, simple and naive implementation for "open text document" this tracks most recent */
let openedDocumentUri: string | null = null;
/**
 * @type {ChildProcessWithoutNullStreams}
 */
let languageServer: ChildProcessWithoutNullStreams | null = null;
let languageServerHandshakeSuccess = false;

type StdoutChunkEntry = { bytesRaw: string | null, bytesDecoded: string };

/** You probably ought to do something more optimal than holding each chunk in memory until you get the entirety. */
let stdoutChunkObjects: StdoutChunkEntry[] = [];
...
let remainingStdoutFromPartiallyReadEvent: string | null = null;
 ...
 * @type {BrowserWindow}
 */
let mainWindowCapture: BrowserWindow;
...
/** TODO: LSP types inheritance */
function MAIN_encodeMessageObject(messageObject: any) {
...
*/
function MAIN_decodeMessage(jsonBytes: string | null) {
	let json;
 ...
 * @returns {string}
 */
function MAIN_decode_experimental_textonly(bytes: Uint8Array, start: number, length: number, EDITOR_lineEndString: string, EDITOR_fileStartsWithBom: boolean) {
 ...
 */
function wrap_readdirSync_indexOf(parentAbsolutePath: string, childFilename: string, childIsDirectory: boolean) {
...
async function didChangeTextDocumentNotification(event: any, absolutePath: string, version: number, startLine: number, startCharacter: number, endLine: number, endCharacter: number, text: string) {
...
async function readAllText(event: any, absolutePath: string) {
...
async function editorDocumentSymbolsRequest(event: any) {
	try {
...
async function editorGoToDefinitionRequest(event: any, indexLine: number, indexColumn: number) {
	try {
		if (!languageServerHandshakeSuccess || !languageServer || !openedDocumentUri) return;
...
async function editorHoverRequest(event: any, indexLine: number, indexColumn: number) {
	try {
		if (!languageServerHandshakeSuccess || !languageServer || !openedDocumentUri) return;
...
async function editorCompletionRequest(event: any, indexLine: number, indexColumn: number) {
	try {
		if (!languageServerHandshakeSuccess || !languageServer || !openedDocumentUri) return;
...
// I don't think 'slice' is in LSP specification but I need to start like this cause it is only way I'll get something "initially working".
async function editorCompletionRequest_slice(event: any, indexStart: number, indexEnd: number) {
	try {
		if (!languageServerHandshakeSuccess || !languageServer || !openedDocumentUri) return;
...
async function setClipboard(event: any, text: string) {
	try {
...
async function editorSetClipboard(event: any, uint8Array: Uint8Array, offset: number, length: number, EDITOR_lineEndString: string) {
	try {
...
async function readClipboard(event: any) {
	try {
...
async function findAll(event: any, search: string, matchWord: boolean) {
	try {
...
async function findAllGetPositions(event: any, absolutePath: string, search: string, matchWord: boolean) {
	if(!isValidAbsolutePath(absolutePath)) return;
 ...
 */
async function newFile(event: any, parentDirectoryAbsolutePath: string, filename: string, isDirectory: boolean) {
 ...
 */
async function deleteFile(event: any, absolutePath: string, isDirectory: boolean) {
 ...
 */
async function renameFile(event: any, absolutePath: string, filename: string, isDirectory: boolean) {
	if (!isValidAbsolutePath(absolutePath)) return;
...
async function saveFile(event: any, absolutePath: string, text: string) {
	if (!isValidAbsolutePath(absolutePath)) return;
...
async function editorSaveFile(event: any, absolutePath: string, uint8Array: Uint8Array, count: number, EDITOR_lineEndString: string, EDITOR_fileStartsWithBom: boolean) {
	if (!isValidAbsolutePath(absolutePath)) return;
 ...
 * ...and other properties as well.
 */
async function copyClipboardAbsolutePathToDirectory(event: any, directory: string, menuOptionCut_id: number) {
